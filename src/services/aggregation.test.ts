...
import { aggregate, detectOutliers, selectStrategy, computeConfidence, applyTemporalSmoothing, mean, median, effectiveSampleSize, normalizeWeights } from './aggregation';
...
function freshProviders(scores: number[], confidences?: number[]): ProviderScore[] {
...
describe('selectStrategy', () => {
  const base = { n: 5, outlierCount: 0, outlierRatio: 0, robustCV: 0.1, meanProviderConfidence: 0.8, effectiveN: 4, maxAgeMinutes: 2 };
  ...
  it('returns median when outlier_ratio >= 0.4', () => {
    expect(selectStrategy({ ...base, n: 5, outlierRatio: 0.5 })).toBe('median');
  });
  it('returns trimmed_mean when outliers present and CV moderate', () => {
    expect(selectStrategy({ ...base, n: 5, outlierRatio: 0.2, robustCV: 0.15 })).toBe('trimmed_mean');
  });
  it('returns median when CV > 0.25', () => {
    expect(selectStrategy({ ...base, n: 10, robustCV: 0.3 })).toBe('median');
  });
  it('returns weighted_mean for high-quality data with n>=7', () => {
    expect(selectStrategy({ ...base, n: 8, robustCV: 0.05, outlierRatio: 0 })).toBe('weighted_mean');
  });
  it('returns trimmed_mean for low confidence with n>=7', () => {
    expect(selectStrategy({ ...base, n: 10, robustCV: 0.1, meanProviderConfidence: 0.5 })).toBe('trimmed_mean');
  });
});

describe('computeConfidence', () => {
  const base = { n: 5, outlierCount: 0, outlierRatio: 0, robustCV: 0.1, meanProviderConfidence: 0.8, maxAgeMinutes: 2, effectiveN: 4 };
  ...
  it('returns lower confidence with outliers', () => {
    const clean = computeConfidence({ ...base, n: 5, outlierRatio: 0, robustCV: 0.1 });
    const withOutliers = computeConfidence({ ...base, n: 5, outlierRatio: 0.4, robustCV: 0.1 });
  ...
  it('returns value in [0, 1]', () => {
    const c = computeConfidence({ ...base, n: 3, outlierRatio: 0.5, robustCV: 0.5, meanProviderConfidence: 0.3 });
...
describe('aggregate', () => {
  ...
  it('uses trimmed_mean when some outliers present', () => {
    ...
    const result = aggregate('crypto', providers);
    expect(result).not.toBeNull();
    expect(['trimmed_mean', 'median', 'weighted_mean', 'bayesian_shrinkage', 'single_provider']).toContain(result!.strategy);
  ...
  it('excludes errored providers from computation', () => {
    const providers = [
      ...
      makeProvider(0, { error: 'timeout', confidence: 0, provider: 'Bad' }),
    ...
    expect(result!.providers.map((p) => p.provider)).toEqual(['Good', 'Bad', 'Good2']);
  ...
  it('excludes stale providers', () => {
    const providers = [
      ...
      makeProvider(70, { confidence: 0.8, timestamp: new Date(Date.now() - 20 * 60_000).toISOString() }),
    ];
    const result = aggregate('stock', providers);
    expect(result).not.toBeNull();
  ...
  it('produces deterministic results for identical inputs', () => {
    const providers = freshProviders([40, 50, 60, 55, 45, 50, 52, 48, 50, 51]);
...
describe('Stress Testing Scenarios', () => {
  ...
  it('STRESS 3: Data gap - providers with varying ages', () => {
    ...
    const providers = [
      makeProvider(80, { confidence: 0.9, timestamp: new Date(Date.now() - 1 * 60_000).toISOString() }),
      makeProvider(20, { confidence: 0.8, timestamp: new Date(Date.now() - 10 * 60_000).toISOString() }),
    ];
    const result = aggregate('stock', providers);
    expect(result).not.toBeNull();
  ...
  it('STRESS 6: Temporal stability across multiple refreshes', () => {
    ...
    expect(Math.abs(result2!.score - result1!.score)).toBeLessThanOrEqual(3);
  });

  it('STRESS 7: All providers failing except one', () => {
    const providers = [
      makeProvider(0, { error: 'timeout', confidence: 0, provider: 'Dead1' }),
      makeProvider(75, { confidence: 0.9, provider: 'Alive' }),
      makeProvider(0, { error: 'api_error', confidence: 0, provider: 'Dead2' }),
    ];
    const result = aggregate('crypto', providers);
    expect(result).not.toBeNull();
    expect(result!.providerCount).toBe(1);
  ...
  it('STRESS 9: Completely empty and missing confidence', () => {
    const providers = [
      { provider: 'A', score: 60, label: 'Neutral' as const, timestamp: new Date().toISOString() },
      { provider: 'B', score: 70, label: 'Greed' as const, timestamp: new Date().toISOString() },
    ];
    const result = aggregate('crypto', providers);
    expect(result).not.toBeNull();
