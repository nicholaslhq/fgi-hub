...
function computeEstimators(
  ...
  const hasOutliers = nonOutlierIndices.length > 0 && nonOutlierIndices.length < n;
  const workingSet = hasOutliers
    ? nonOutlierIndices.map((i) => ({ score: scores[i], weight: weights[i] }))
...
function assessQuality(
  ...
  const robustCV =
    medianVal !== 0 && !Number.isNaN(medianVal) && mad > 0 && !Number.isNaN(mad)
  ...
  const maxAgeMinutes = n > 0 ? Math.max(...providers.map((p) => p.ageMinutes)) : 0;
...
function selectStrategy(q: QualityMetrics): AggregationStrategy {
  ...
  if (q.n >= MEDIUM_N) {
    ...
    if (q.meanProviderConfidence < LOW_CONFIDENCE_WARN) return 'trimmed_mean';
...
function computeScore(
  ...
  const { weightedMean: wm, median: med, trimmedMean: tm, bayesian } = estimators;
...
function computeConfidence(q: QualityMetrics): number {
  if (q.n === 0) return 0;
  if (q.n === 1) {
    return clamp(0.15 + 0.3 * (q.meanProviderConfidence - 0.5) + 0.15, 0, 1);
  }
...
function computeConfidenceInterval(
  ...
  if (useRobustSE && mad > 0) {
    ...
    const sigma = Math.sqrt(weightedVariance(scores, normWeights, wm || medianVal));
...
export function aggregate(
  ...
  const timestamps = weighted.map(
    (p) => new Date(p.provider.timestamp).getTime(),
  );
