...
export function ProviderConsensus({ data, market, theme }: { data: ConsensusResult; market: 'stock' | 'crypto'; theme: 'light' | 'dark' }) {
  const activeScores = data.providers.filter((p) => !p.error).map((p) => p.score);
  ...
  const staleCount = data.providers.filter((p) => !p.error && isStale(p.timestamp)).length;
  const activeCount = data.providers.length - errorCount - staleCount;
  const confidencePct = data.confidence !== undefined ? Math.round(data.confidence * 100) : 0;
  ...
  return (
    <div className="card animate-fade-in" style={{ animationDelay: '0.2s' }}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <h3 className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-text-tertiary)' }}>
          Provider Consensus — {market === 'stock' ? 'Stocks' : 'Crypto'}
        </h3>
        <div className="consensus-meter">
          <span className="text-xs font-medium" style={{ color: divergenceColor[divergence] }}>
      ...
      <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-4 text-xs">
        <div className="flex items-center gap-2">
          <span style={{ color: 'var(--color-text-tertiary)' }}>Strategy:</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-medium" style={{ color: confidenceColor }}>
            {confidencePct}%
          </span>
        </div>
        {data.ciLower !== undefined && data.ciUpper !== undefined && (
          <div className="flex items-center gap-2">
            <span style={{ color: 'var(--color-text-tertiary)' }}>95% CI:</span>
            <span className="font-mono font-medium" style={{ color: 'var(--color-text-primary)' }}>
      ...
      <div className="space-y-1">
        {data.providers.map((provider) => (
          <ProviderRow key={provider.provider} provider={provider} theme={theme} />
        ))}
      </div>
      <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-6 pt-5" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
        <div className="flex items-center gap-2">
