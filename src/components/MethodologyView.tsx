...
function ScoreScale({ theme }: { theme: "light" | "dark" }) {
	const levels = [
		...
		{ ...sentimentRange("Extreme Greed"), label: "Extreme Greed", score: 90 },
	];
			...
			{levels.map((level) => (
				<div key={level.label} className="flex items-center gap-4">
					<div
						...
						style={{ background: getSentimentColor(level.score, theme) }}
					/>
	...
	return (
		...
		>
			...
			<div className="space-y-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
				<p>
					<span className="font-medium" style={{ color: 'var(--color-text-tertiary)' }}>Definition: </span>
					{definition}
				</p>
				<p>
					<span className="font-medium" style={{ color: 'var(--color-text-tertiary)' }}>Calculation: </span>
					<code className="font-mono text-xs" style={{ color: 'var(--color-text-primary)' }}>
						{calculation}
					</code>
				</p>
				<p>
					<span className="font-medium" style={{ color: 'var(--color-text-tertiary)' }}>Interpretation: </span>
					{interpretation}
				</p>
			</div>
		</div>
...
export function MethodologyView() {
	...
	return (
		<div className="space-y-8 animate-fade-in">
			<div className="text-center max-w-2xl mx-auto mb-12">
				...
				<p
					...
					style={{ color: 'var(--color-text-secondary)' }}
				>
					FGI Hub uses an Adaptive Robust Aggregation (ARA) framework that
					dynamically selects the optimal consensus strategy for each market
					based on real-time data quality, distribution characteristics, and
		...
		<Section title="What is the Fear & Greed Index?">
			<p>
				The Fear & Greed Index is a sentiment indicator that measures the
				emotional state of market participants. It ranges from 0 (Extreme
				Fear) to 100 (Extreme Greed), helping investors identify potential
				market tops and bottoms driven by emotion.
			</p>
			<p>
				FGI Hub aggregates this index across multiple independent data
				providers for both stock and cryptocurrency markets, producing a
				more robust and less biased reading than any single source.
			</p>
		</Section>
		...
		<Section title="Markets Covered">
			<p>
				FGI Hub currently tracks two distinct markets:
			</p>
			<ul className="list-disc list-inside space-y-2 mt-2">
				...
				<li>
					...
					— Cryptocurrency market sentiment across leading digital assets.
				</li>
			</ul>
			<p>
				Because stocks and crypto often exhibit different risk
				appetites, FGI Hub reports them separately while also providing
				an aggregated cross-market view.
			</p>
		</Section>

		<Section title="Data Freshness">
			<p>
				Provider data is refreshed on demand. Scores are timestamped,
				and any provider that has not updated within the last 15
				minutes is excluded from the consensus. Within the freshness
				window, newer data carries proportionally more weight via
		...
		<Section title="Adaptive Robust Aggregation (ARA)">
			<p>
				The ARA framework operates in five sequential phases. At each
				phase, the algorithm characterizes the current data state and
				selects the most appropriate mathematical strategy — all
				decisions are deterministic, data-driven, and reproducible.
			</p>
			...
			<div className="space-y-4 pt-2">
				<div>
					...
					<p
						...
						style={{ color: 'var(--color-text-secondary)' }}
					>
						Each provider result is first validated (non-error, in-range,
						valid timestamp). Valid providers are then assigned a
						reliability weight combining three factors:
					</p>
					<p
						className="text-sm mt-1"
						style={{ color: 'var(--color-text-secondary)' }}
					>
						<code
							...
							style={{ color: 'var(--color-text-primary)' }}
						>
							wᵢ = confidenceᵢ × e^(−ageᵢ / τ) × staleness_penaltyᵢ
						</code>
					</p>
					<p
					...
					>
						Where τ = 5 min sets the recency half-life, confidence comes
						from the provider, and staleness penalty gradually reduces weight
						for providers aged beyond 70% of the freshness window.
					</p>
				</div>
				...
				<div>
					...
					<p
						...
						style={{ color: 'var(--color-text-secondary)' }}
					>
						Using the Median Absolute Deviation (MAD), the framework
						computes robust z-scores for each provider. Values exceeding
						a z-score of 3.5 are flagged as outliers — this is resistant
						to masking effects that plague standard deviation-based methods.
					</p>
				</div>
				...
				<div>
					...
					<p
						...
						style={{ color: 'var(--color-text-secondary)' }}
					>
						The framework computes four candidate estimators in parallel:
					</p>
				...
				<div>
					...
					<p
						...
						style={{ color: 'var(--color-text-secondary)' }}
					>
						Six quality signals are computed from the data: provider count,
						outlier ratio, robust coefficient of variation (MAD/median),
						mean provider confidence, effective sample size (Kish's
						formula), and maximum data age.
					</p>
				</div>
				...
				<div>
					...
					<p
						...
						style={{ color: 'var(--color-text-secondary)' }}
					>
						Based on the quality signals, the framework deterministically
						selects the most appropriate estimator. The decision rules are
						functions of data characteristics, not hardcoded to any single
						market condition:
					</p>
					<div className="space-y-2 mt-2">
						<div
							...
							style={{ color: 'var(--color-text-tertiary)' }}
						>
							<strong style={{ color: 'var(--color-text-primary)' }}>n &lt; 3</strong>
							→ Bayesian shrinkage (small-sample regularization)
						</div>
						<div
							className="text-xs"
							style={{ color: 'var(--color-text-tertiary)' }}
						>
							<strong style={{ color: 'var(--color-text-primary)' }}>Outlier ratio &ge; 40%</strong>
						...
						<div
							...
							style={{ color: 'var(--color-text-tertiary)' }}
						>
							<strong style={{ color: 'var(--color-text-primary)' }}>Robust CV &gt; 0.25</strong>
						...
						<div
							...
							style={{ color: 'var(--color-text-tertiary)' }}
						>
							<strong style={{ color: 'var(--color-text-primary)' }}>Outliers present, CV &gt; 0.20</strong>
						...
						<div
							...
							style={{ color: 'var(--color-text-tertiary)' }}
						>
							<strong style={{ color: 'var(--color-text-primary)' }}>Otherwise</strong>
		...
		<Section title="Confidence &amp; Uncertainty">
			<p>
				...
				interval computed using robust statistics (MAD-based standard
				error for robust strategies, Kish-weighted standard error
				for parametric strategies).
			</p>
			<div
			...
			>
				Confidence = (n/(n+2)) × (1−CV) × (1−2×outlier_ratio) × mean_confidence × (effective_n/n) × stability
			</div>
			<p
				className="text-xs mt-1"
				style={{ color: 'var(--color-text-tertiary)' }}
			>
				Each factor is clamped to [0, 1] and the product is clamped to
				[0, 1]. A score of 0.90 means high certainty that the true
				consensus falls within the reported range.
			</p>
		</Section>
		...
		<Section title="Metrics Dictionary">
			<p className="mb-6">
				Every metric displayed in the Markets tab is derived from the
				provider score dataset. Below is a comprehensive reference for
				each indicator, including its mathematical definition and
