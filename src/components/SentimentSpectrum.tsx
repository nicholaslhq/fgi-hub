...
export function getSentimentColor(score: number, theme: 'light' | 'dark'): string {
  const colors =
    theme === 'dark'
      ? {
...
export function SentimentSpectrum({ score, theme }: { score: number; theme: 'light' | 'dark' }) {
  return (
      ...
      <div className="spectrum-track">
        ...
        <div className="spectrum-marker" style={{ left: `${score}%` }}>
          ...
          <div
            ...
            style={{
              '--pulse-color': getSentimentColor(score, theme),
              backgroundColor: getSentimentColor(score, theme),
...
export function ScoreDisplay({ score, label, market, timestamp, size = 'lg', theme }: {
  score: number;
  label: string;
  market: string;
  timestamp: string;
...
export function ProviderRow({ provider, theme }: { provider: ProviderScore; theme: 'light' | 'dark' }) {
  ...
  return (
    <div className="provider-row">
      <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
        <div className={`status-dot ${status}`} title={provider.error || 'Healthy'} />
        <div className="min-w-0">
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-tertiary)' }}>
            {provider.error ? (
              <span style={{ color: 'var(--color-fear)' }}>{provider.error}</span>
            ) : (
