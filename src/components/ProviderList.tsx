        ...
        {providers.map((p) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className={`w-2.5 h-2.5 rounded-full ${sentimentDot(p.score, p.error)}`} />
              <div>
            ...
            <div className="text-right">
              <p className={`text-sm font-bold ${p.error ? 'text-text-muted line-through' : 'text-text-primary'}`}>
                {p.error ? '—' : p.score}
              </p>
