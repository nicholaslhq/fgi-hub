...
export function ConsensusCard({ title, data }: { title: string; data: ConsensusResult | null }) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-text-secondary mb-4">{title}</h2>
      {data ? (
        <div className="flex flex-col items-center gap-4">
          <div className="relative w-40 h-40">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-bold text-text-primary">{data.score}</span>
              <span className="text-xs text-text-muted mt-1">/ 100</span>
            </div>
          </div>
          <div className="text-center">
            <p className={`text-xl font-bold ${sentimentTextColor(data.score)}`}>{data.label}</p>
            <p className="text-sm text-text-muted mt-1">
              Updated {formatTime(data.lastUpdated)}
            </p>
            <p className="text-xs text-text-muted mt-0.5">
              {data.providerCount} provider{data.providerCount !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
      ) : (
